...
Nothing yet calls `TcpChannel::connect`/`::accept`, `BluetoothChannel::connect`/`::accept`, `DiscoveryService::spawn_listener`, or `connect_best_available` outside their own tests — no pairing handshake is wired to any of them (that's track G7) — so two daemons can't actually find and pair with each other over the network today, but the TCP transport, both discovery mechanisms, the Bluetooth transport, and the negotiation step are all real, tested to the extent this environment allows (`cargo test -p flow-daemon --lib channel::tcp channel::negotiate discovery::tcp`; `cargo test -p flow-daemon --features bluetooth --lib channel::bluetooth discovery::bluetooth`).

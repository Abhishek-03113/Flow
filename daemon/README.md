...
**Status: every task in [`todos.json`](./todos.json) (tracks A-J) is done.** In order of what actually runs: `flow-daemon` binds a WebSocket Control Link listener on `127.0.0.1:47823`, serves the full `docs/contracts/daemon-ipc.md` contract (backed by `flow-core`'s mock-parity `DaemonService`) over it, and persists settings/devices/identity/history to a local SQLite database (tracks A/P/B/C). `flow-platform` has real per-OS capture/injection for Linux (evdev/uinput), macOS (`CGEventTap`/`CGEventPost`), and Windows (`SetWindowsHookEx`/`SendInput`) — track E — with the local switch-key hotkey (F1-F3) wired to actually flip the active device without needing the UI running at all. Two daemons can discover each other over TCP or Bluetooth, negotiate a medium, complete a real pairing handshake, and stream keyboard/mouse input end to end while the pairing is `Active` (track G); that connection is authenticated (a Noise session bound to each side's persisted ed25519 identity) and gated against a trust store before any input can flow, with a replay guard on the receiving side (track H); a dropped connection re-negotiates and reconnects with backoff, every state-changing command is traced, and a panic in one connection handler can't take down another (track I). Track J closes the loop: the workspace lints clean, the full test suite and cross-compile checks are documented and verified, service-unit scaffolding exists for macOS/Linux, and this file (see the reconciliation note in its final section) has been checked against the actual code rather than left aspirational.

**What "done" does not mean here**, stated plainly rather than left to be inferred: several real, substantial pieces of end-to-end wiring were deliberately left for a later phase, each documented at its own point below — no incoming-connection accept loop runs inside `main.rs` itself yet (so two independently-*started* `flow-daemon` processes can't yet find each other without a test harness gluing the pieces together), `NoiseChannel`/the trust gate/auto-reconnect are real, tested, standalone building blocks not yet spliced into that same production accept path, and everything beyond Linux capture/injection is verified by cross-compilation and unit tests only — this project's own development container has neither a second machine nor the OS-native input hardware (`/dev/input`, `/dev/uinput`, a Bluetooth adapter) to run any of it against for real. None of this is a gap discovered *by* J5 — it's the same honest boundary every task that hit it documented as it landed; J5's job was to confirm that boundary is still accurately described, not to close it.
...
```
core/       flow-core     — protocol, device, pairing, channel, state, input traits (no OS/Channel/UI deps)
daemon/     flow-daemon   — this binary; wires flow-core + flow-platform together, owns the Control Link server
...
This binds a WebSocket listener on `127.0.0.1:47823` (`docs/contracts/daemon-ipc.md`'s local Control Link contract) and stays running, serving commands and pushing state events, until interrupted (Ctrl-C).
...
[`docs/contracts/`](../docs/contracts) defines the exact interface — commands, state streams, error codes, state machines — that `flutter/lib/data/mock_daemon_repository.dart` implements on the Flutter side. `flow-daemon` is the second, real implementation of that same contract, reachable over the local Control Link: `flutter/lib/state/repository_providers.dart`'s `daemonRepositoryProvider` can point at it instead of the mock via `--dart-define=FLOW_DAEMON_MODE=ipc`, with **no UI code changing** (`docs/contracts/README.md` ground rule 2; see `flutter/README.md` "Running against a real daemon"). `daemon/todos.json` tracks A-D built exactly this, in order.
...
`daemon/src/hotkey/` detects the configured switch-key combination directly from the platform's real input capture and triggers a device switch **without any Control Link client connected**, per `vision.md` §8 ("Daemon Works without UI"). Two pieces:
...
- `hotkey::runner::spawn` (`runner.rs`, track F2) — starts `flow_platform::DefaultInputCapture` (whichever real per-OS adapter this binary was built for), bridges its event stream through the matcher, and calls `DaemonService::switch_active_device_local()` on a match — a separate, error-free path from the Control Link `switch_active_device` command (track C), since a raw key press has no "requester" to reject with an error. Spawned alongside the Control Link listener and history logger in `main.rs`.

**The hotkey runner degrades gracefully, not fatally**, when the platform adapter can't start (no capturable device, missing permission): it logs a warning and the daemon keeps serving the Control Link normally without it. Confirmed in this container, which has no `/dev/input` at all:

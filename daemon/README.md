...
The check happens when the request actually arrives, not before the Noise handshake: the window can close while the handshake is still in flight, and a request landing after the user gave up is exactly the unattended case being refused.

**What this is not:** within an open window, pairing is still first-come-first-served. There's no short-authentication-string or numeric-comparison step proving the device that answered is the one the user meant, so a hostile host racing a genuine peer during those few seconds could still win. Closing that needs a real accept/reject prompt, which needs a new command in `docs/contracts/daemon-ipc.md` (whose `PairingSession` only models the *initiating* side) plus UI to show it. This narrows exposure from "always, unattended" to "a few seconds, while the user is deliberately pairing" — a large reduction, not a complete fix.

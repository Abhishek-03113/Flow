...
`platform/src/macos/injector.rs` (`MacosInputInjector`) posts synthetic `CGEvent`s built from incoming `InputEvent`s via `CGEventPost`, through the pure(-ish) `inject_translate::to_cg_event` function (the reverse of E4's `EventTranslator`; "pure-ish" since building a `CGEvent` is a real Core Graphics call, not just struct construction, but it needs no tap or permission). `MouseEvent::Move` carries a relative delta, but `CGEvent::new_mouse_event` wants an absolute position *and the window server moves the cursor from that absolute location* — so `move_cg_event` reads the cursor's current position (`CGEvent::new(source).location()`) and posts the event at `current + delta`. The `MOUSE_EVENT_DELTA_X`/`Y` fields are also set, for apps that read raw deltas (games, pointer-lock). The original E5 code set *only* those delta fields and left the location at the current cursor position, which meant every injected move told the cursor to stay exactly where it already was — the pointer never moved, and left-click (fired at the stale position) missed whatever the sender was pointing at while right-click still popped a context menu somewhere, so it looked like "left click is broken." Two related fixes in the same pass: `button_cg_event` now sets `MOUSE_EVENT_CLICK_STATE = 1` (AppKit `-mouseDown:` handlers ignore a synthetic click without it), and a `Move` arriving while a button is held is posted as the matching `*MouseDragged` type rather than `MouseMoved` (`MacosInputInjector` tracks held buttons across events via `HeldButtons`), so text selection and drag-and-drop follow. Verified with `cargo check -p flow-platform --target x86_64-apple-darwin` and `--target aarch64-apple-darwin` (both `cargo check` and `clippy --all-targets -D warnings`, per E5's acceptance criteria specifying `aarch64-apple-darwin`), plus `cargo fmt` — no macOS hardware exists here, so `inject_translate.rs`'s unit tests (constructing `InputEvent`s and asserting on the resulting `CGEvent`'s type/fields, the same style as E4's tests) have never actually executed:

...
- `flow-daemon` — a `main.rs` that constructs `AppState` and prints a placeholder line. No Control Link, no command handling.
...
A  core-contract-parity ─> P  persistence-storage (SQLite) ─┬─> B  daemon-command-service ─> C  local-ipc-transport ─> D  flutter-ipc-adapter
                                                              ...
                                                              └─> E  platform-input-adapters ─┬─> F  switch-hotkey ─┐
                                                                                                │                    ├─> G  channels (TCP + Bluetooth) ─> H  security-pairing-trust ─┐
                                                                                                └────────────────────┘                                                               │
...
**A -> P -> B -> C -> D is the priority path.** It's the Rust equivalent of what the Flutter phase did with the mock: get a fully contract-correct, durably-stored, no-OS-dependencies service running and reachable over the wire before touching a single platform API. Track B's `DaemonService` reuses `MockDaemonRepository`'s exact seed data and pairing timings as a *first-run bootstrap* on purpose — the point isn't a different mock, it's proving the *same* observable behavior is implementable in Rust, backed by a real database, and reachable from Flutter over a real Control Link. D is the payoff: swap `daemonRepositoryProvider` from `MockDaemonRepository` to `IpcDaemonRepository` and nothing in `lib/features/` should need to change.
...
**E (platform input) runs in parallel with B/C**, not after — it only depends on A (the shared `InputEvent`/`HostOs` vocabulary), not on the service or Control Link layer. Linux (E1-E3) is buildable and unit-testable in this container; macOS/Windows (E4-E7) are cross-compile-checked only, since there's no Mac or PC available here — flagged explicitly in each task's acceptance criteria so "done" never silently means "assumed to work."
...
**G (channels) needs E and F** — you can't stream real input between two daemons until each one can capture/inject locally and knows when it's the active one. G is where daemon-to-daemon connectivity is built as a single custom abstraction — a **Channel** — that can be backed by TCP (Wi-Fi/local network) or Bluetooth depending on what's actually available between two devices; see [`docs/architecture/channels.md`](../docs/architecture/channels.md) for the full design and G1-G8 below for how it's built up (abstraction → TCP → discovery → Bluetooth → discovery → negotiation → pairing → streaming). **H (security) needs G** — there's nothing to encrypt until there's a real Channel, and H3's encryption wraps the `Channel` trait generically so it works over either medium without caring which. **I (reliability)** cuts across C/G and is scheduled after them rather than interleaved, since retry/persistence logic is much easier to write correctly against a working Channel than to build alongside one.
...
| **G** — channels | 8 | E, F | A medium-agnostic `Channel` abstraction (TCP + Bluetooth) exists; two daemons discover, negotiate a medium, pair, and stream real input end to end (Linux, local test) |
| **H** — security-pairing-trust | 4 | G, P | Daemon-to-daemon traffic is identity-keyed (persisted), Noise-encrypted (over any Channel), replay-protected |
| **I** — reliability-persistence | 3 | C, G, P | Link state recovers from a dropped connection by renegotiating a Channel; daemon panics are isolated per-task |
...
- **Every third-party crate doing real I/O sits behind a project-owned abstraction — no exceptions.** `rusqlite` behind `Storage`/`*Repo` (P), `tokio-tungstenite` behind `TcpChannel` (G2) and separately behind the local Control Link handler (C), a Bluetooth crate behind `BluetoothChannel` (G4), `snow` behind `NoiseChannel` (H3), `evdev`/`core-graphics`/`windows` behind `InputCapture`/`InputInjector` (E). Application/service code never imports a third-party crate directly outside its one wrapper module — an explicit open/closed-principle rule (`architecturalPrinciples.wrapThirdPartyDependencies` in `todos.json`), checked per-task (e.g. "grep for tokio_tungstenite outside daemon/src/channel/tcp.rs comes back empty"), not left aspirational.
- **Daemon-to-daemon connectivity is a single abstraction named "Channels."** A `Channel` is established between two daemons over TCP (Wi-Fi/local network) or Bluetooth, whichever is available — one trait, two implementations, a negotiation step that picks between them, and encryption/pairing/streaming written once against the trait rather than per-medium. Full design in [`docs/architecture/channels.md`](../docs/architecture/channels.md), deliberately maintained separately from `docs/contracts/` (that directory is the Flutter<->daemon boundary; Channels is daemon<->daemon) but in the same documentation style so the two stay easy to reconcile if Channel state ever needs to surface into the Flutter contract later.
...
- **Mock-parity timings are deliberately copied into `DaemonService`** (1200ms/1500ms/1600ms) even though `daemon-ipc.md` says they're not part of the contract for a *production* daemon. The reason is development ergonomics during this phase: the observable behavior of "mock" vs. "real daemon over the Control Link" should be indistinguishable while D is being built and tested, so a swap never looks like a regression. Track I or a later phase can detach real timings from these constants once actual network/pairing latency exists.

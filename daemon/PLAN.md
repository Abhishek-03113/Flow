...
```
A  core-contract-parity     ─┬─> B  daemon-command-service ─> C  local-ipc-transport ─> D  flutter-ipc-adapter
                              │
...
**A -> B -> C -> D is the priority path.** It's the Rust equivalent of what the Flutter phase did with the mock: get a fully contract-correct, in-memory, no-OS-dependencies service running and reachable over the wire before touching a single platform API. Track B's `DaemonService` reuses `MockDaemonRepository`'s exact seed data and pairing timings on purpose — the point isn't a different mock, it's proving the *same* observable behavior is implementable in Rust and reachable from Flutter over real IPC. D is the payoff: swap `daemonRepositoryProvider` from `MockDaemonRepository` to `IpcDaemonRepository` and nothing in `lib/features/` should need to change.
...
| **B** — daemon-command-service | 7 | A | An in-memory `DaemonService` passes a test suite mirroring the Dart mock's 13 cases |
...
| **H** — security-pairing-trust | 4 | G | Daemon-to-daemon traffic is identity-keyed, Noise-encrypted, replay-protected |
| **I** — reliability-persistence | 4 | C, G | Settings/devices persist across restarts; link state recovers from a dropped connection |

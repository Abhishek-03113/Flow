...
| Ownership role + generation | `daemon/src/ownership.rs` (`OwnershipHandle`), shared by `DaemonService`, `pipeline::run_paired_connection`, `hotkey::runner::spawn_pipeline_switch_filter` | Explicit `InputRole` (not derived from `devices` — a fresh daemon and a told-Secondary daemon both leave their own device `Active`, so the `Active` flag alone can't tell them apart) + a monotonic `u64` generation embedded in `ChannelMessage::SwitchOwnership`. A receiver applies the message only if `generation` is strictly greater than the last one accepted (`OwnershipHandle::try_advance_generation`); an equal or lower value is a stale/duplicate retransmit and is dropped without touching role, forwarding, or suppression. Defaults `Primary`, in-memory only, reset to `Primary` when a pipeline ends (so a Secondary whose peer vanished can still initiate on the next connection). `DaemonService::{local_role, is_local_primary, is_local_secondary, peer_is_primary}` expose it. |

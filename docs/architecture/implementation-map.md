...
| Self-injection guard | Linux: skip own uinput node. macOS: `EVENT_SOURCE_USER_DATA` marker. Windows: `dwExtraInfo` marker (`FLOW_INJECTED_MARKER`), hooks skip matching events. | Without it a slave re-captures its own injected output and echoes it back to the active peer; relative `MouseMove` deltas compound. Windows guard added product-first V1 iter 3 after `drive_two_daemons` amplified 14 events → 212k. |
...
| Switch during a live connection | `hotkey/runner.rs` `spawn_pipeline_switch_filter` + `service.enter_peer_pipeline()` / `peer_pipeline_active()` | "Option A": while a peer pipeline runs it owns switch authority (its capture stream still sees the key even while the OS hook withholds it), strips the switch key's KeyDown **and** matching KeyUp from the forwarded stream, and the standalone runner stands down. **Primary-only gate** ("Complete Flow V1" task §6): a match only calls `switch_active_device_local()` when `service.is_local_primary()`; a Secondary's own press is still consumed (never forwarded) but logs `stage="switch_ignored" reason="not_primary"` and does not switch. |
| Ownership role + generation | `daemon/src/ownership.rs` (`OwnershipHandle`), shared by `DaemonService`, `pipeline::run_paired_connection`, `hotkey::runner::spawn_pipeline_switch_filter` | Explicit `InputRole` (not derived from `devices` — a fresh daemon and a told-Secondary daemon both leave their own device `Active`, so the `Active` flag alone can't tell them apart) + a monotonic `u64` generation embedded in `ChannelMessage::OwnershipChanged { primary_device_id, generation }` — an **absolute** device id, never a sender-relative role a receiver has to invert. A receiver applies a live handoff only if `generation` is strictly greater than the last one accepted (`OwnershipHandle::try_advance_generation`); an equal or lower value is a stale/duplicate retransmit and is dropped without touching role, forwarding, or suppression. A `primary_device_id` naming neither this daemon nor its peer is rejected outright (`reason=invalid_target_owner`). Defaults `Primary`, in-memory only. **Not** reset on disconnect ("Fix Flow V1 Ownership Synchronization" task — an unconditional reset on both peers was the split-brain bug): every new connection, including a reconnect, opens with both sides exchanging their current belief and independently resolving any disagreement via `pipeline::resolve_ownership` (`OwnershipHandle::reconcile`) — higher generation wins, an exact tie breaks deterministically by device id. `DaemonService::{local_role, is_local_primary, is_local_secondary, peer_is_primary}` expose it. |
| Switch state transition | `service/mod.rs` `switch_active_device` (IPC) / `switch_active_device_local` (hotkey) | Both maintain the single-`Active` invariant; both emit a `stage="switch"` `hop_note!`. The IPC path is **not** Primary-gated (only the Scroll Lock path is, per task §6's concrete scope) — a known, documented limitation, not an oversight. |

...
**Status: both concrete implementations and both discovery mechanisms have landed (G1-G5); negotiation (G6) is next.** The `Channel` trait and its vocabulary (`ChannelKind`, `ChannelAddress`, `ChannelMessage`, `ChannelError`) exist in `core/src/channel/mod.rs` exactly as drafted here — replacing the earlier `core::transport::Transport` placeholder. `TcpChannel` (`daemon/src/channel/tcp.rs`) and `BluetoothChannel` (`daemon/src/channel/bluetooth.rs`, Linux-only, behind the `bluetooth` Cargo feature) both implement it against real sockets. `discovery::tcp` and `discovery::bluetooth` both produce `DiscoveredPeer` values. Nothing yet calls any of this outside its own tests — no pairing handshake is wired to a `Channel` (that's G7) — so two daemons can't actually find and pair with each other over the network today, and `connect_best_available` (G6, below) hasn't landed yet either. See `daemon/README.md`'s Channels section for exactly what is and isn't verified in this project's own development environment. Version **0.1.0**.
...
`NoiseChannel` (`daemon/todos.json` H3) is a `Channel` that wraps *any other* `Channel`, layering an authenticated [Noise protocol](http://www.noiseprotocol.org/) handshake (via the `snow` crate) keyed by each side's persisted identity (`daemon/todos.json` P6/H1). Because it's written against the `Channel` trait rather than a concrete medium, encryption is identical whether the inner channel is `TcpChannel` or `BluetoothChannel` — one implementation, not two.

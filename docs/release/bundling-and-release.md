...
**Status: draft, not yet implemented.** Written now, ahead of code, for the same reason `docs/architecture/channels.md` and `docs/testing/manual-testing-strategy.md` were: so the shape of the work is agreed before it's built rather than improvised at release time. `docs/tasks/daemon-todos.json` track **J** (`packaging-qa`) is the closest existing track to this — J4 scaffolds service-unit files, J3 covers cross-compilation — but neither track owns "how do we actually ship this to a user." This document does.
...
- No `packaging/` directory exists yet — `docs/tasks/daemon-todos.json` J4 (launchd/systemd/Windows-service scaffolding) hasn't landed.
- `docs/tasks/daemon-todos.json` tracks A/P/B/C/D are done (real daemon, real IPC, real persistence); E (platform input), F (switch-hotkey), G (Channels/networking), H (security), I (reliability), J (packaging) are not.

A release strategy written against a daemon that can't yet capture input or talk to another machine would be fiction. So this document is a **ladder**, not a single "how to ship v1.0.0" recipe — each rung is tied to what `docs/tasks/daemon-todos.json` actually needs to be true first, and to the manual verification gates `docs/testing/manual-testing-strategy.md` already defines.
...
- The daemon is registered as a **systemd user unit** (`docs/tasks/daemon-todos.json` J4's `.service` file, installed to `~/.config/systemd/user/` and enabled via `systemctl --user enable --now`), consistent with not requiring root for a per-user background utility.
...
**Real installer builds happen on that platform's own OS, not cross-compiled from this Linux development environment.** This mirrors the same honesty rule already applied to E4-E7 in `docs/tasks/daemon-todos.json`: `cargo check --target x86_64-pc-windows-msvc` proves the Rust code compiles, but producing an actual signed, working Windows installer needs the real MSVC toolchain, and `flutter build windows` needs to run on Windows in the first place (Flutter's desktop build tooling isn't cross-compiling in the way a plain Rust binary can). Concretely: the Linux artifacts (`.deb`/`.AppImage`) are the only ones buildable in this project's current development environment; macOS/Windows artifacts need a real Mac/PC (or, eventually, CI runners of that OS — not set up yet, and out of scope for this document per its own framing at the top).

## Versioning

**One version number for the whole product**, not independently versioned Flutter/Rust halves — `Cargo.toml`'s `workspace.package.version` and `flutter/pubspec.yaml`'s `version` are bumped together, in the same commit, every release. They already happen to both read `0.1.0`; this document turns that into an enforced policy rather than a coincidence. (A CI check that fails if they diverge is a natural fit for whenever `docs/tasks/daemon-todos.json`/a Flutter equivalent covers CI — not built yet, noted here as the eventual enforcement mechanism.)
...
Skipping a rung (e.g., shipping a macOS build before Tier 5 has actually happened on real Apple hardware) is the specific mistake this ladder exists to prevent — cross-compile-checked (`cargo check --target ...`, per `docs/tasks/daemon-todos.json` E4-E7) is not the same claim as "verified," and a release should never imply otherwise.
...
**Explicitly out of scope for v1.0.0** (matches the project's habit, established in `docs/tasks/daemon-todos.json` and `docs/architecture/channels.md`, of stating exclusions rather than leaving them implicit):
...
- CI/CD automation itself (a GitHub Actions build-and-release pipeline). This document describes what such a pipeline would need to produce; building the pipeline is future work, tracked wherever `docs/tasks/daemon-todos.json`'s J track (or a new one) picks it up — not written here per the instruction that release docs live in `docs/release/`, not as actual workflow files.

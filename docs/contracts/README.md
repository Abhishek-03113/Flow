...
**Status: implemented.** Contract version `0.1.0` (medium decided in `0.1.1`) is realized by both `MockDaemonRepository` (Flutter-side stand-in) and `flow-daemon` (`daemon/src/ipc/{dispatch,server}.rs`, over the WebSocket medium documented in `daemon-ipc.md`), which must agree with each other on every JSON shape below.

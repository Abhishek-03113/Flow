...
3. **This is the local IPC contract, not the wire protocol between two daemons.** `docs/product/vision.md` §11 (Input Event Protocol) and `core/src/protocol` define the keyboard/mouse event format daemons exchange with each other over the network transport. That protocol and this one are independent; a UI never sees a raw `InputEvent`.

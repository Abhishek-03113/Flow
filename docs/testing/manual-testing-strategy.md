...
- [ ] Tray popover shows exactly one device — this machine's own real hostname/OS, `Active` — and nothing else. Seeing "Work Laptop"/"Desktop"/"MacBook" instead means you're silently still on the mock, or looking at a stale `flow-daemon` build predating "Removing mock runtime data" (`daemon/README.md`).
...
- [ ] Start pairing from the UI on a fresh daemon with no second machine running: confirm "Pair New Device" reaches `Found` with an **empty** candidate list, not the old mock's "Office Mac Mini"/"Studio Linux" — production never seeds fake candidates now. Pairing a real second daemon (Tier 3+, once track G lands end to end) is what actually populates this list.
...
- [ ] Run the existing automated cross-language contract test (`daemon/README.md` "Cross-language contract test") — this exercises the mock-parity scenarios against a real daemon explicitly started with `FLOW_DAEMON_SEED_MOCK_PARITY=1` (a real deployment never sets this); run it once per work session as a fast regression check before doing anything manual.

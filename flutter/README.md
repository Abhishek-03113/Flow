...
- The in-app logo: `core/widgets/app_logo.dart`'s `AppLogo`/`AppLogoMark` already render `assets/flow_logo.png` everywhere branding shows up (onboarding's welcome step and standalone header) and fall back to the old accent square until that file exists — drop a real `flow_logo.png` into `assets/` and it's picked up with no further code changes.

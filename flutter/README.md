...
**Exception:** with `--dart-define=FLOW_ENV=development` (see below), the window's close button quits the process for real instead of hiding it. Hide-on-close is deliberate for the shipped app, but during `flutter run` iteration it means the *previous* run's process is often still alive in the background (hidden, tray-only) when you launch the next one — which can surface as `flutter run` logging `Failed to foreground app; open returned 1` (macOS Launch Services refusing to bring the fresh build forward because an old instance answering to the same app identity is still running), and whoever's testing ends up looking at the stale instance's state instead of the one that just launched — easy to mistake for "the daemon connection is broken." If you ever suspect this, fully quit Flow (its tray's "Quit Flow", or `killall flow_ui`) before the next `flutter run`.

By default this expects a real `flow-daemon` already listening on `127.0.0.1:47823` (`flow_core::ipc::IPC_PORT`) — without one, every provider stays in `AsyncLoading`/errors out. See "Running without a daemon" below to work on the UI alone. `IpcDaemonRepository` and `daemonRepositoryProvider` both log to the `flutter run` console (`flow-daemon: daemon backend = ...`, `flow-daemon: connecting to ...` / `connected to ...`) — worth checking first when something looks off, since "on the mock backend" and "on IPC but can't reach flow-daemon" look identical in the UI but need different fixes.
